...
func (cli *DockerCli) CmdBuild(args ...string) error {
	cmd := cli.Subcmd("build", "PATH | URL | git:// | hg:// | -", "Build a new image from the source code at PATH", true)
	tag := cmd.String([]string{"t", "-tag"}, "", "Repository name (and optionally a tag) for the image")
	suppressOutput := cmd.Bool([]string{"q", "-quiet"}, false, "Suppress verbose output generated by the containers")
	noCache := cmd.Bool([]string{"#no-cache", "-no-cache"}, false, "Do not use cache when building the image")
	rm := cmd.Bool([]string{"#rm", "-rm"}, true, "Remove intermediate containers after successful build")
	...
	daemonClone := cmd.Bool([]string{"-daemon-clone"}, false, "Clone the source repository (git|hg) in the daemon")
	...
	if cmd.Arg(0) == "-" {
	...
	} else if urlutil.IsURL(cmd.Arg(0)) &&
		(*daemonClone || (urlutil.IsGitURL(cmd.Arg(0)) && !hasGit) ||
			(urlutil.IsHgURL(cmd.Arg(0)) && !hasHg) ||
			!(urlutil.IsGitURL(cmd.Arg(0)) || urlutil.IsHgURL(cmd.Arg(0)))) {
		isRemote = true
	} else {
		root := cmd.Arg(0)
		if urlutil.IsGitURL(root) {
			...
			if output, err := exec.Command("hg", "clone", urlutil.CleanHgURL(remoteURL), root).CombinedOutput(); err != nil {
				return fmt.Errorf("Error trying to use hg: %s (%s)", err, output)
			}
		}
